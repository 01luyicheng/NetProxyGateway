...
func TestIPFilter_IsAllowed(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "allowed IP in 10.0.0.0/8",
			ip:       "10.0.0.1",
			expected: true,
		},
		{
			name:     "allowed IP in 172.16.0.0/12",
			ip:       "172.16.0.1",
			expected: true,
		},
		{
			name:     "allowed IP in 192.168.0.0/16",
			ip:       "192.168.1.1",
			expected: true,
		},
		{
		...
		{
			name:     "public IP outside allowed",
			ip:       "8.8.8.8",
			expected: false,
		},
		{
			name:     "IP just outside 172.16.0.0/12",
			ip:       "172.32.0.1",
			expected: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := filter.IsAllowed(tt.ip)
			if result != tt.expected {
				t.Errorf("IsAllowed(%q) = %v, want %v", tt.ip, result, tt.expected)
			}
		})
	}
}
