...
## 2024-06-25 - Accessible and smooth async buttons
**Learning:** Hardcoding spinner colors inside disabled buttons causes poor contrast, and abrupt state swaps during async operations break visual continuity.
**Action:** Wrap button inner elements in `AnimatedContent` (along with an explicit `Row` to preserve layout) for smooth transitions, and use `LocalContentColor.current` for loading spinners to automatically inherit correct disabled styling.
