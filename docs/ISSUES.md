...
## REV59: PR #108 误删的安全/正确性防护栏（多子项） [已修复]

> 以下问题由多个 subagent 对 `git diff origin/dev...origin/jules-16778843622874874870-dedfe299`（PR #108 分支）进行深度审查发现。PR #108 在重构 `DebugDetector`、`MqttConnectionManager`、`VpnPacketProcessor`、`server/api/main.go` 与 `.github/workflows/ci.yml` 时，误删了若干安全关键防护栏。本条目逐项记录回归与修复。
>
...
- **修复方式**: 恢复 `resolveDebugPropertiesState` 循环的双路径 OR 语义——对每条属性同时调用 `readPropertyValueViaReflection(prop, getMethod)` 与 `processPropertyReader(prop)`，`if (isDebugViaReflection || isDebugViaProcess) return true`。恢复 `isDebugPropertyValue` 与 `readPropertyValueViaReflection` helper。保留 PR #108 的 `readDebugPropertyValue` helper（仅用于"反射优先、失败回退"语义场景，不参与安全关键检测）。恢复 dev 测试 `resolveDebugPropertiesState_detectsDebugViaProcess_whenReflectionIsHooked`（Frida 绕过守卫）。**同类回归**：dev 分支 REV47。
...
- **修复方式**: 恢复 `@Volatile var workloadFingerprint` 与 `@Volatile var timingCheckInvocationCount`（含 KDoc），恢复 `checkTimingAttack` 的 `now: () -> Long = { System.currentTimeMillis() }` 可注入时钟参数、`workloadFingerprint = sum` 与 `timingCheckInvocationCount++` 副作用写入，以及完整 KDoc。恢复 dev 测试 `checkTimingAttack_executesObservableWorkload`（DCE 守卫）与时钟注入测试。**同类回归**：PR #95。
...
- **修复方式**: 恢复 `readProcessOutput` 的 reader-thread + `process.waitFor(PROCESS_TIMEOUT_SECONDS, ...)` + `destroyForcibly().waitFor(...)` 回收结构；`readPropertyViaProcess` 委托给 `readProcessOutput(listOf("getprop", prop))`。恢复 dev 测试 `readProcessOutput_returnsNull_whenProcessHangsWithoutOutput`（挂起守卫）。**同类回归**：dev 分支 REV46。
...
- **修复方式**: 恢复 `buildCorsConfig`、`parseCORSAllowedOrigins`、`defaultCORSAllowedOrigins` 函数与 `corsConfig = buildCorsConfig(os.Getenv("API_ALLOWED_ORIGINS"))` 调用。env var 未设置时保留 localhost 默认值以保证本地开发可用。不引入 `AllowAllOrigins` 或 `AllowOrigins: ["*"]`（与 credentials 不兼容）。恢复 PR #125 的 11 条 CORS 回归测试（含 `TestCORSIntegration_EnvVarAllowsProductionOrigin` 与 `TestCORSIntegration_DisallowedOriginStillRejectedAfterEnvSet` 安全守卫）。
