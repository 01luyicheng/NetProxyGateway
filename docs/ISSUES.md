...
### REV25: MqttConnectionManager `_connectionState.value = Connecting` 竞态导致状态机永久卡死
- **状态**: 已修复
...
### REV26: server/tunnel Register/Unregister 未检查 stopped 导致 WaitGroup 重用 panic
- **状态**: 已修复
...
- **风险**: **高**。服务关闭时可能 panic
- **修复方式**: 在 `Register` 和 `Unregister` 中，在 `m.wg.Add(1)` 前获取 `m.stopMu` 并检查 `m.stopped`，若已停止则跳过 `wg.Add(1)` 和 goroutine 启动

### REV27: server/tunnel Register/Unregister 在 m.mu 锁内调用 tunnel.Close() 执行阻塞 I/O
- **状态**: 已修复
...
### REV28: server/api GET /api/pair/:code 无认证可枚举配对码
- **状态**: 已修复
...
### REV29: server/api updatePairingSession TOCTOU 竞态可致会话劫持
- **状态**: 已修复
- **位置**: `server/api/main.go` (L835-918)
...
- **修复方式**: 使用原子条件 UPDATE（`WHERE engineer_id = '' OR engineer_id = ?`），检查 `RowsAffected()` 是否为 1。若为 0 说明会话已被其他工程师占用，返回 409 Conflict

### REV30: SOCKS5 代理域名连接绕过 IP 验证（安全漏洞）
- **状态**: 已修复
