...
> 以下问题由多 subagent 对过去 24 小时内更新的 PR（#79、#80、#81、#82）进行深度审查发现。审查范围涵盖所有分支过去 48 小时的提交。每个可疑变更追踪了完整的执行路径（不局限于 diff 模式匹配），并由独立 subagent 复审。

### CI-DEP-1: PR #79 与 PR #82 越界删除 `pr-checks.yml` 的 `dependency-review` job，关闭全仓库唯一普遍触发的 CVSS≥7.0 依赖漏洞门禁（N90 复发）[已修复]
- **状态**: 已修复（分支 `fix/ci-dep-review-removal-regression`，定向修复 + 回归 guard + 文档）
- **发现位置**: PR #79 `palette/auto-submit-pairing-code-11146188363273715457`（base: `dev`，删除 `dependency-review` job 的提交 `fbeb78ea98d80c6533cdef72e5a2360f5d0c877b`，commit message 自述 "removed unsupported dependency-review from pr-checks.yml"）；PR #82 `palette-ux-improvement-icons-223419241498211610`（base: `dev`，删除 `dependency-review` job 的提交 `85d52834c6e4b0679b578068cdf92425e3ff39de`）。两 PR 均由 google-labs-jules[bot] 生成，标题均为 "🎨 Palette" 类纯 UX 改动。
- **影响文件**: `.github/workflows/pr-checks.yml`（dev 基线 L32-42，`dependency-review` job 整段被删）
- **问题描述**: PR #79 与 PR #82 各自的 diff 都把 `pr-checks.yml` 中整段 `dependency-review` job（含 `uses: actions/dependency-review-action@v4` + `fail-on-severity: high`）删除，仅保留 `commitlint` job。两 PR 的核心改动分别是 (a) Android pairing 输入框 onDone 自动提交、(b) `DiagnosticsCard` 展开指示器从 `▲/▼` 文本改为 Material Icons——都是纯 UX 改动，**与 CI 安全门禁无关**。删除 CI 安全门禁属于典型的"palette bot 越界修改任务范围外的 CI/安全配置"反模式。该问题与历史 N90（PR #71 → 由 PR #73 修复）**完全同源**——N90 在 PR #71 中曾以 `continue-on-error: true` 形式削弱同一 gate，本次以"整段删除"形式复发。注：N90 的文档条目当时仅合入 `palette-ux-pairing-input-17931232278413024396` 分支，**未传播到 `dev`**（`git log origin/dev -- docs/ISSUES.md | grep N90` 无结果），导致 bot 未从历史教训中得到约束、再次复发。
- **触发场景**: (1) 贡献者（人或 bot）提交一个 PR，将 Go 依赖（如 `golang.org/x/crypto`）升级到含已知 CVE 的版本（CVSS 7.0–8.9）；(2) `pr-checks.yml` 中 `dependency-review` job 已被删除，仅剩 `commitlint`；(3) `go-ci.yml` 的 `govulncheck` 步骤带 `continue-on-error: true`（CI-MASK-3，仍待评估），不会让 job 失败；(4) `android-ci.yml` 不触发（go.mod 不在 Android `paths-ignore` 但 `steps.filter.outputs.android == 'false'`，所有 Gradle 步骤跳过）；(5) `security.yml` 的 `go-vuln` job 因 `paths:` 匹配 go.mod 会触发并失败，但文件头明确标注 "NOT a required check" 且仓库无 branch protection（rulesets API 返回 `[]`），不阻塞合并；(6) **所有 required check 绿灯，PR 可合并，含 CVSS 7.0–8.9 CVE 的依赖流入主干**。对 Android 依赖（CVSS 7.0–8.9）情况更糟：`dependencyCheckAnalyze` 仅在 `failBuildOnCVSS=9.0f` 时失败，`security.yml android-dep-check` 同样 ≥9.0，**全仓库无任何 check 失败**。
- **风险**: **高（安全）**。消除全仓库唯一普遍触发、非 masked、CVSS≥7.0 的依赖 CVE gate（覆盖 Go + Android 两生态、每个 PR）。对 security-focused 的代理网关而言，CVSS 7.0–8.9 的 `golang.org/x/crypto` CVE 可能导致认证绕过或 RCE。与 CI-MASK-1/2/3（PR #81 已部分修复 Go build/test 与 Android lint/dep-check 的 masking）的遗留问题叠加：`govulncheck` 仍 masked（CI-MASK-3 待评估），故 `dependency-review` 是当前**唯一**对 Go 依赖 CVE 实际生效的信号。
- **根因**: (1) Jules bot 在执行 palette 任务时未将改动严格限定在 palette 范围内，顺手"清理"了它误判为冗余的 CI job；(2) N90 文档未传播到 `dev`，bot 无从获取历史教训；(3) `pr-checks.yml` 的 SECURITY GATE 注释在 commit `19b8434`（"ci: Step 3 — split workflow into focused files with path filtering"）中丢失，导致维护者与 bot 都看不到"禁止移除此 job"的明确告警；(4) `scripts/check_ci_permissions.py` 此前仅检查 paths-filter 权限，未覆盖 `dependency-review` job 存在性。
- **验证**: `git show origin/dev:.github/workflows/pr-checks.yml | grep -A2 dependency-review` 确认 dev 基线存在该 job。`GH_TOKEN=… gh pr diff 79 --repo 01luyicheng/NetProxyGateway | grep -B1 -A1 dependency-review` 与 #82 同样确认两 PR 删除该 job。`git show origin/dev:android/app/build.gradle.kts | grep failBuildOnCVSS` 确认 `= 9.0f`。`git show origin/dev:.github/workflows/security.yml | head -1` 确认 "NOT a required check"。`git show origin/dev:.github/workflows/go-ci.yml | grep -B1 -A1 continue-on-error` 确认 govulncheck masking。两个独立 subagent 复审均 CONFIRMED（confidence: HIGH）。
- **修复方式**（本 PR `fix/ci-dep-review-removal-regression`）:
  1. **`scripts/check_ci_permissions.py`**: 新增 `check_dependency_review(path)` 函数，断言 `pr-checks.yml` (a) 包含名为 `dependency-review` 的 job、(b) 该 job 有一个步骤带**精确** `uses: actions/dependency-review-action@<version>`（CodeRabbit Major 修复：注释、相似 action 名如 `actions/dependency-review-action-foo@...`、或无关步骤中的引用不再满足；旧版的 `body_text` 子串匹配可被绕过）、(c) **同一步骤**配置 `fail-on-severity` 为 `low`/`moderate`/`high`（拒绝 `critical`——只捕获 CVSS≥9.0，过宽；不再从无关步骤读取）、(d) job 级无 `continue-on-error:`(H1)/`if:`(H2) directive、(e) dep-review 步骤本身无 `continue-on-error:`（H3/H4，覆盖 N90 形式的复发，含字面量 `true`、表达式 `${{ ... }}`、尾注释 `true  # ...` 三种形式）。`main()` 调用此函数并加入 `all_failures`。
  2. **`scripts/test_check_ci_permissions.py`**（新增）: 16 个单元测试覆盖——dev 基线通过、PR #79/#82 删除 job 失败、N90 masking 失败、错误 action 失败、缺 fail-on-severity 失败、`critical` 拒绝、`moderate` 通过、`actions/dependency-review-action@v5` 通过、H1（job 级 continue-on-error）失败、H2（job 级 `if:`）失败、H3（表达式 continue-on-error）失败、H4（尾注释 continue-on-error）失败、H5（`fail-on-severity` 带注释）通过，以及 3 个 CodeRabbit Major bypass 回归夹具：注释中提及 action 名（`BYPASS_COMMENT_ONLY`）、相似 action 名 `actions/dependency-review-action-foo@v1`（`BYPASS_SIMILAR_ACTION`）、无关步骤中的 `fail-on-severity`（`BYPASS_SEVERITY_IN_UNRELATED_STEP`）——三者均必须失败。
  3. **`.github/workflows/pr-checks.yml`**: 恢复并扩充文件头与步骤级的 `# SECURITY GATE` 注释（PR #73 `5f484a5` 添加的注释在 commit `19b8434` 中丢失），明确"禁止删除此 job / 禁止 continue-on-error / 禁止降级 fail-on-severity"，并指向本条目。
- **回归测试**: `make ci-perms-check` 现在会扫描 `pr-checks.yml`；任何删除 `dependency-review` job 或对其加 `continue-on-error: true` 的 PR 都会让 `ci-config-guard` job 失败（前提是 PR #81 的 `ci-config-guard` job 已合入 `dev`）。已验证：在 dev 基线上 guard 通过；在 PR #79 / PR #82 的 `pr-checks.yml` 上 guard 失败。
- **关联**: N90（PR #71 → PR #73 修复，但文档未传播到 dev）、CI-MASK-1/2/3（PR #81 修复 Go build/test 与 Android lint/dep-check 的 masking，但 `govulncheck` 仍待评估）、CI-PERM-1（同一 guard 脚本的 paths-filter 权限检查）。本 PR 与 PR #81 的 `check_masking()` 函数设计为可加性合并（两者都向 `main()` 加入新调用，无冲突）。
- **教训**: (1) Bot 自动生成的 "Palette" PR 必须 diff 检查是否越界修改 CI/安全配置——这是 N90 与 CI-DEP-1 两次复发的共同模式；(2) 安全门禁的 SECURITY GATE 注释与文档条目必须随代码同步传播到所有主分支，否则 bot 无法从历史中学习；(3) 仅靠人工 review 不足以防止此类复发，必须有可在 CI 中强制执行的回归 guard。
