...
- **发现位置**: PR #79 `palette/auto-submit-pairing-code-11146188363273715457`（base: `dev`）；PR #82 `palette-ux-improvement-icons-223419241498211610`（base: `dev`）。两 PR 均由 google-labs-jules[bot] 生成，标题均为 "🎨 Palette" 类纯 UX 改动。
...
- **修复方式**（本 PR `fix/ci-dep-review-removal-regression`）:
  1. **`scripts/check_ci_permissions.py`**: 新增 `check_dependency_review(path)` 函数，断言 `pr-checks.yml` (a) 包含名为 `dependency-review` 的 job、(b) 该 job 使用 `actions/dependency-review-action@<any-version>`、(c) 配置 `fail-on-severity` 为 `low`/`moderate`/`high`（拒绝 `critical`——只捕获 CVSS≥9.0，过宽）、(d) 该步骤不得带 `continue-on-error: true`（覆盖 N90 形式的复发）。`main()` 调用此函数并加入 `all_failures`。
  2. **`scripts/test_check_ci_permissions.py`**（新增）: 8 个单元测试覆盖——dev 基线通过、PR #79/#82 删除 job 失败、N90 masking 失败、错误 action 失败、缺 fail-on-severity 失败、`critical` 拒绝、`moderate` 通过、`actions/dependency-review-action@v5` 通过。
